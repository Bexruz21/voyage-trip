...
function Footer() {
    ...
    const contactInfo = [
        ...
        },
        {
            ...
            description: 'info@voyagetrip.uz'
        },
        {
            icon: MapPin,
            text: t('footer.addressTitle'),
            href: '#',
            description: t('footer.address')
        }
    ];
    ...
    return (
        <footer className="bg-gradient-to-br from-gray-50 via-blue-50 to-cyan-50 border-t border-gray-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                    <div className="flex items-center space-x-3">
                        <img src="/logo.png" alt="VOYAGE TRIP" className="w-16 h-16" />
                        <div>
                            <h2 className="text-xl font-bold text-gray-900">VOYAGE TRIP</h2>
                            <p className="text-gray-600">{t('nav.logo')}</p>
                        </div>
                    </div>
            ...
            <div className="border-t border-gray-200 py-6 text-center text-gray-600 text-sm">
                © {currentYear} VOYAGE TRIP. {t('footer.rights')}
                <span className="hidden sm:inline mx-2">•</span>

                <Link
                    href="/license"
                    className="hidden lg:inline hover:text-cyan-600 transition-colors underline"
                >
                    {t('footer.licenseFull')}
                </Link>
            </div>
        </footer>
    );
